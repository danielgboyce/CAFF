...
##GET RELEVANT BITS
fs$nfisheries<-rowSums(fs[,6:11])
fs$ngears<-rowSums(fs[,31:56])
fs$nmanag<-rowSums(fs[,58:71])
fs$comanaged<-fs[,73]
fs$usr<-fs[,18]
fs$lrp<-fs[,19]
fs$mortality<-fs[,84]
fs$biomass<-fs[,86]
fs$rebuild<-fs[,107]
fs$moratorium<-fs[,112]
fs$cosewic<-fs[,145]
fs$cosewicstatus<-fs[,146]
fs$bycatch<-fs[,114]
fs$bycatchretained<-fs[,124]
fs$ifmp<-fs[,117]

##a<-subset(fs,comanaged=='Yes')
...
names(fs)[1:6]<-c('stock.orig','stock','speciesg','region','status','harm')
